...
apply_cinnamon() {
    info "Applying ${THEME_NAME} to Cinnamon..."
...
apply_gtk_settings() {
    info "Applying ${THEME_NAME} via GTK settings file..."
        ...
        for gtk_dir in "${gtk3_dir}" "${gtk4_dir}"; do
            ...
            if [[ -f "${settings_file}" ]]; then
                ...
                if grep -q "^gtk-application-prefer-dark-theme=" "${settings_file}"; then
                    sed -i "s|^gtk-application-prefer-dark-theme=.*|gtk-application-prefer-dark-theme=${prefer_dark_value}|" "${settings_file}"
                else
...
main() {
    ...
    echo ""
    success "${THEME_NAME} theme applied successfully!"
