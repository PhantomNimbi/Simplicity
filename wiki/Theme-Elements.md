...
`Simplicity-Icons` is a Windows 11-inspired icon theme shipped alongside the GTK variants. All icons are scalable SVGs (`scalable/`), making them sharp at any size from 16 × 16 to 256 × 256. The theme inherits from `hicolor` so any icon not yet provided falls back gracefully to the system default.

### Design Language

The icon set follows Microsoft's **Fluent Design** language as seen in Windows 11:

| Principle | Implementation |
|-----------|---------------|
| Rounded shapes | Rounded-rectangle app backgrounds (`rx="10"` on 48 × 48) |
| Vibrant colour | Per-context accent colours (blue, golden-orange, green, etc.) |
| Soft gradients | Linear gradients from lighter top to darker bottom |
| Consistent weight | Uniform `stroke-width` and shape sizing across contexts |
| Transparency | White icons with `opacity` overlays for depth |

### Colour Reference

| Context | Primary colour | Usage |
|---------|---------------|-------|
| Places / Folders | `#FFD04A` → `#FFA000` | Folder gradient (golden-orange) |
| Apps | Per-app (blue, orange, green, grey…) | Rounded-rect app background |
| Actions | `#5294e2` (Simplicity accent) | Toolbar and action icons |
| Status — Info | `#0078D4` | Dialog information, network icons |
| Status — Warning | `#FFB900` | Dialog warning triangle |
| Status — Error | `#D13438` | Dialog error, delete |
| Status — Question | `#0078D4` | Dialog question mark |

### Icon Contexts

#### `scalable/places/` — Place icons

| Icon name | Description |
|-----------|-------------|
| `folder` | Default golden folder |
| `folder-open` | Open golden folder (back flap visible) |
| `folder-home` | Home folder — house badge |
| `folder-downloads` | Downloads folder — down-arrow badge |
| `folder-documents` | Documents folder — document badge |
| `folder-pictures` | Pictures folder — landscape photo badge |
| `folder-music` | Music folder — music-note badge |
| `user-trash` | Empty trash can (grey) |
| `user-trash-full` | Full trash can — paper sticking out |

#### `scalable/apps/` — Application icons

| Icon name | Background | Description |
|-----------|-----------|-------------|
| `utilities-terminal` | `#1E1E2E` dark | Terminal with chevron prompt |
| `text-editor` | Blue `#42A5F5` | Document with text lines |
| `system-file-manager` | Orange `#FFD04A` | Folder icon |
| `preferences-system` | Grey `#78909C` | Gear with 8 teeth |
| `web-browser` | Teal `#29B6F6` | Globe with latitude/meridian lines |
| `system-software-install` | Green `#66BB6A` | Package box with down-arrow |

#### `scalable/actions/` — Action / toolbar icons

All action icons use the Simplicity accent blue (`#5294e2`) on a transparent
background for clean toolbar and menu integration.

| Icon name | Visual |
|-----------|--------|
| `document-new` | Blank document with `+` |
| `document-open` | Folder with up-arrow (load) |
| `document-save` | Classic floppy disk |
| `edit-copy` | Two overlapping documents |
| `edit-paste` | Clipboard with content lines |
| `edit-cut` | Scissors |
| `edit-delete` | Red circle with `×` |
| `edit-undo` | Counter-clockwise arc with arrow |
| `edit-redo` | Clockwise arc with arrow |
| `go-home` | House silhouette |
| `view-refresh` | Circular arrow |
| `list-add` | Green circle with `+` |

#### `scalable/status/` — Status / indicator icons

| Icon name | Colour | Symbol |
|-----------|--------|--------|
| `dialog-information` | `#0078D4` blue circle | `i` |
| `dialog-warning` | `#FFB900` yellow triangle | `!` |
| `dialog-error` | `#D13438` red circle | `×` |
| `dialog-question` | `#0078D4` blue circle | `?` |
| `network-wireless` | `#5294e2` | Three fan arcs + dot |

#### `scalable/mimetypes/` — File-type icons

All mimetype icons share a white document base with a folded-corner indicator
and a colour-coded top bar matching the file category.

| Icon name | Bar colour | Content badge |
|-----------|-----------|---------------|
| `text-x-generic` | — | Horizontal text lines |
| `image-x-generic` | `#64B5F6` blue | Landscape with mountains and sun |
| `audio-x-generic` | `#CE93D8` purple | Music note |
| `video-x-generic` | `#42A5F5` blue | Filled play-button circle |
| `application-x-executable` | `#66BB6A` green | `< >` code brackets |
